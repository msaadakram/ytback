...
export const getSoundCloudInfo = wrapAsync(async (req, res) => {
    const { url } = req.validated;
    const info = await fetchInfo(url, 'soundcloud');
...
export const downloadSoundCloudVideo = wrapAsync(async (req, res) => {
    const { url, formatId, quality, container } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'soundcloud');
        title = info.title || info.description || 'soundcloud-audio';
    } catch {
    ...
    res.status(202).json({ success: true, data: { jobId: job.id, platform: 'soundcloud', status: 'started' } });
});

export const downloadSoundCloudAudio = wrapAsync(async (req, res) => {
