...
export const getPinterestInfo = wrapAsync(async (req, res) => {
    const { url } = req.validated;
    const info = await fetchInfo(url, 'pinterest');
...
export const downloadPinterestVideo = wrapAsync(async (req, res) => {
    const { url, formatId, quality, container } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'pinterest');
        title = info.title || info.description || 'pinterest-video';
    } catch {
    ...
    res.status(202).json({ success: true, data: { jobId: job.id, platform: 'pinterest', status: 'started' } });
});

export const downloadPinterestAudio = wrapAsync(async (req, res) => {
    const { url, format, quality } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'pinterest');
        title = info.title || info.description || 'pinterest-audio';
    } catch {
