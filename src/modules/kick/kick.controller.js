...
export const getKickInfo = wrapAsync(async (req, res) => {
    const { url } = req.validated;
    const info = await fetchInfo(url, 'kick');
...
export const downloadKickVideo = wrapAsync(async (req, res) => {
    const { url, formatId, quality, container } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'kick');
        title = info.title || info.description || 'kick-video';
    } catch {
        title = 'kick-video';
    }
    ...
    res.status(202).json({ success: true, data: { jobId: job.id, platform: 'kick', status: 'started' } });
});

export const downloadKickAudio = wrapAsync(async (req, res) => {
    const { url, format, quality } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'kick');
        title = info.title || info.description || 'kick-audio';
    } catch {
        title = 'kick-audio';
    }
