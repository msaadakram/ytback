...
export const getTwitchInfo = wrapAsync(async (req, res) => {
    const { url } = req.validated;
    const info = await fetchInfo(url, 'twitch');
...
export const downloadTwitchVideo = wrapAsync(async (req, res) => {
    const { url, formatId, quality, container } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'twitch');
        title = info.title || info.description || 'twitch-video';
    } catch {
        title = 'twitch-video';
    }
    ...
    res.status(202).json({ success: true, data: { jobId: job.id, platform: 'twitch', status: 'started' } });
});

export const downloadTwitchAudio = wrapAsync(async (req, res) => {
    const { url, format, quality } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url, 'twitch');
        title = info.title || info.description || 'twitch-audio';
    } catch {
        title = 'twitch-audio';
    }
