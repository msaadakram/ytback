...
export const getBilibiliInfo = wrapAsync(async (req, res) => {
...
export const downloadBilibiliVideo = wrapAsync(async (req, res) => {
    const { url, formatId, quality, container } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url);
        title = info.title || info.description || 'bilibili-video';
    } catch {
        title = 'bilibili-video';
    }

    const job = jobStore.create({ type: 'video', platform: 'bilibili', url, formatId, quality, container, title });
    ...
    res.status(202).json({ success: true, data: { jobId: job.id, platform: 'bilibili', status: 'started' } });
});

export const downloadBilibiliAudio = wrapAsync(async (req, res) => {
    const { url, format, quality } = req.validated;

    let title;
    try {
        const info = await fetchInfo(url);
        title = info.title || info.description || 'bilibili-audio';
    } catch {
        title = 'bilibili-audio';
    }

    const job = jobStore.create({ type: 'audio', platform: 'bilibili', url, format, quality, title });
